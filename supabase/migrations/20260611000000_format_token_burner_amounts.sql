-- 토큰 수 표기를 웹과 동일한 K/M/B/T 단위로 통일한다
create or replace function public.format_token_amount(amount bigint)
returns text
language sql
immutable
set search_path = ''
as $$
  select case
    when abs(amount) >= 1000000000000 then to_char(amount / 1000000000000.0, 'FM999990.0') || 'T'
    when abs(amount) >= 1000000000 then to_char(amount / 1000000000.0, 'FM999990.0') || 'B'
    when abs(amount) >= 1000000 then to_char(amount / 1000000.0, 'FM999990.0') || 'M'
    when abs(amount) >= 1000 then to_char(amount / 1000.0, 'FM999990.0') || 'K'
    else to_char(amount, 'FM999,999')
  end;
$$;

update public.badges
set description = '하루에 토큰 50M 이상을 사용했습니다.'
where badge_key = 'token-burner';

-- 이미 부여된 토큰 버너 배지의 기록도 새 표기로 변환한다
update public.user_badges ub
set evidence_summary = format(
  '하루 최대 %s tokens',
  public.format_token_amount(
    nullif(regexp_replace(ub.evidence_summary, '\D', '', 'g'), '')::bigint
  )
)
from public.badges b
where b.id = ub.badge_id
  and b.badge_key = 'token-burner'
  and ub.evidence_summary ~ '\d';
