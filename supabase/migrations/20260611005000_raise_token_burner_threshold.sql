-- 토큰 버너 기준을 하루 50M -> 100M으로 상향하고,
-- 에이전트별 프롬프트 수를 위한 provider 단위 턴 합산 RPC를 추가한다
update public.badges
set description = '하루에 토큰 100M 이상을 사용했습니다.'
where badge_key = 'token-burner';

create or replace function public.get_provider_turn_totals()
returns table (provider text, total_turns bigint)
language sql
stable
set search_path = ''
as $$
  select s.provider, coalesce(sum(s.user_turn_count), 0) as total_turns
  from public.usage_sessions s
  where s.user_id = auth.uid()
  group by s.provider;
$$;

revoke execute on function public.get_provider_turn_totals() from public, anon;
grant execute on function public.get_provider_turn_totals() to authenticated;
